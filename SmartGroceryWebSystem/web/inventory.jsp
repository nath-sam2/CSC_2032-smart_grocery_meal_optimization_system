...
/* SIDEBAR (same as dashboard/products) */
.sidebar{
...
<h1>🏭 Inventory</h1>
<p>Track your household stock levels, low-stock alerts and expiry warnings.</p>
</div>
...
<td class="qty-cell <%= i.isLowStock() ? "qty-low" : "" %>"><%= i.getQuantity() %></td>
<td><%= i.getReorderLevel() %></td>
<td>
<% if (i.isLowStock()) { %>
<span class="badge badge-low"><i class="fa-solid fa-triangle-exclamation"></i> Low Stock</span>
<% } else { %>
<span class="badge badge-ok"><i class="fa-solid fa-circle-check"></i> OK</span>
<% } %>
</td>
</tr>
<%
...
<tr>
<td>#<%= i.getProductId() %></td>
<td class="qty-cell qty-low"><%= i.getQuantity() %></td>
<td><%= i.getReorderLevel() %></td>
...
<td><%= p.getName() %></td>
<td style="color:#f59e0b;font-weight:700;"><%= p.getExpiryDate() %></td>
